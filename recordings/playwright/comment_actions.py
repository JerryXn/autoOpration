...
def handle_all_comments(page):
    try:
        ...
        try:
            ...
            from auto_opration_python.src.storage.sql_repo import insert_comments_skip, insert_batch_log
            url = page.url or ''
...
def scan_until_total(page, max_rounds: int = 300):
    ...
    while rounds < max_rounds:
        ...
        try:
            sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
            sys.path.insert(0, str(Path(__file__).resolve().parents[2] / 'auto_opration_python'))
            from auto_opration_python.src.storage.sql_repo import insert_comments_skip, insert_batch_log, insert_note_skip
            url = page.url or ''
            nid = _current_note_id(page)
            if nid:
        ...
        try:
            ...
            if end_cnt and end_cnt > 0:
                try:
                    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
                    sys.path.insert(0, str(Path(__file__).resolve().parents[2] / 'auto_opration_python'))
                    from auto_opration_python.src.storage.sql_repo import insert_batch_log, insert_note_skip
                    url = page.url or ''
                    nid = _current_note_id(page)
                    if nid:
                        insert_note_skip(nid, None, url)
